  ...
  return (
    <section id="contact" className="border-t border-(--border) dark:border-(--border) px-6 py-24 lg:px-8 lg:py-40">
      <div className="mx-auto max-w-7xl">
        <p className="text-sm font-medium uppercase tracking-[0.2em] text-(--accent)">{t('contact_label')}</p>

        <h2 className="mt-6 max-w-4xl text-5xl font-semibold leading-none tracking-[-0.04em] md:text-7xl">{t('contact_title')}</h2>

        <p className="mt-8 max-w-xl text-base leading-7 text-(--muted) dark:text-(--subtle)">{t('contact_paragraph')}</p>
        ...
        <a href="https://wa.me/5511988032511" target="_blank" className="group mt-10 inline-flex items-center gap-3 text-lg font-medium text-(--foreground) transition hover:text-(--foreground) dark:text-(--subtle) dark:hover:text-(--foreground)">
          <Phone size={20} />
          (11) 9 8803-2511
          <ArrowUpRight size={18} className="transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" />
        </a>
